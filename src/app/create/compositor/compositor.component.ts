 ...
 * measure the exact markup it mounts (see compositor.paginate.ts). Editing on
...
import { countWords, gaps } from './compositor.analysis';
import { PAGE, margin, pagesHtml } from './compositor.paginate';
...
@Component({
  ...
  standalone: true,
  imports: [CounterComponent, RailComponent, InspectorComponent, TimelineComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './compositor.component.html',
  /* The state attributes live on the HOST, not on `.app`. The stylesheet
     selects `.cmp[data-stage='intake'] .work`, so an attribute one level
...
export class CompositorComponent implements AfterViewInit, OnDestroy {
